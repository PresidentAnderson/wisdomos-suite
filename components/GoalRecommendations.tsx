  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-black">Analyzing your data and generating recommendations...</div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {analysis && (
        <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
          <h3 className="text-xl font-semibold text-black mb-4">Your Profile Analysis</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              <div className="text-sm text-gray-600">Dominant Moods</div>
              <div className="flex gap-2 mt-1">
                {analysis.dominantMoods.map(mood => (
                  <span key={mood} className="px-2 py-1 bg-white/20 rounded text-xs text-black">
                    {mood}
                  </span>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="text-sm text-gray-600 mb-2">Your Strengths</div>
              <div className="flex flex-wrap gap-2">
                {analysis.strengths.length > 0 ? (
                  ...
                  <span className="text-gray-600 text-sm">Keep journaling to identify strengths</span>
                )}
              </div>
            </div>
      ...
      <div>
        <h3 className="text-xl font-semibold text-black mb-4">AI-Powered Goal Recommendations</h3>
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 ${getDifficultyColor(recommendation.difficulty)} rounded text-xs text-black`}>
                      {recommendation.difficulty}
                    </span>
                    <span className="px-2 py-1 bg-white/20 rounded text-xs text-black">
                      {recommendation.timeframe}
                    </span>
                  </div>
                </div>
                
                <h4 className="text-lg font-semibold text-black mb-2">{recommendation.title}</h4>
      ...
      {selectedGoal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl p-6 max-w-md w-full">
            ...
            <h3 className="text-xl font-semibold text-black mb-2">{selectedGoal.title}</h3>
            ...
            <div className="space-y-3 mb-4">
              ...
              <div>
                <div className="text-sm text-gray-600 mb-1">Related to Your Focus Areas</div>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex items-center gap-4">
                <div>
                  ...
                  <div className="text-black capitalize">{selectedGoal.difficulty}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Timeframe</div>
                  <div className="text-black">{selectedGoal.timeframe}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Category</div>
                  <div className="text-black capitalize">{selectedGoal.category}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-500 text-black py-2 rounded-lg hover:from-cyan-600 hover:to-blue-600 transition-all"
              >
                Adopt This Goal
              </button>
              <button
                onClick={() => setSelectedGoal(null)}
                className="flex-1 bg-white/20 text-black py-2 rounded-lg hover:bg-white/30 transition-all"
              >
                Maybe Later
              </button>
            </div>
