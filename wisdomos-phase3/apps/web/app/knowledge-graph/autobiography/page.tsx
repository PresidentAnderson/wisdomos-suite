...
export default function AutobiographyKnowledgeGraphPage() {
  ...
  const loadClusters = async () => {
    try {
      ...
      const response = await fetch("/api/knowledge-graph/cluster-tags?graphType=autobiography", {
        headers: {
  ...
  const handleExport = async () => {
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("authToken") : "";
      const response = await fetch("/api/knowledge-graph/export?graphType=autobiography", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `autobiography-graph-${Date.now()}.json`;
        a.click();
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-gradient-to-br from-phoenix-orange to-phoenix-red rounded-xl flex items-center justify-center">
                <Book className="w-6 h-6 text-white" />
              </div>
              <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-phoenix-orange to-phoenix-gold bg-clip-text text-transparent">
                  Autobiography Knowledge Graph
                </h1>
                <p className="text-gray-600">
                  Explore thematic patterns across your life story
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <motion.div
            ...
            className="lg:col-span-3 bg-white rounded-xl shadow-lg border border-phoenix-gold/20 overflow-hidden"
