          ...
          return (
            ...
            >
              ...
              >
                <Icon 
                  className={`w-6 h-6 ${
                    isActive ? 'text-black' : isPast ? 'text-black' : 'text-black'
                  }`}
                />
              ...
              <div className="mt-3 text-center">
                <h4 className={`text-sm font-semibold ${
                  isActive ? 'text-black' : isFuture ? 'text-black' : 'text-black'
                }`}>
                  {stage.name.split(' ')[0]}
                </h4>
              ...
              {interactive && (
                <motion.div
                  className="absolute -top-20 left-1/2 transform -translate-x-1/2 bg-white/95 backdrop-blur-lg rounded-lg p-3 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity w-48 z-10"
                  initial={{ opacity: 0, y: 10 }}
                  whileHover={{ opacity: 1, y: 0 }}
                >
                  <p className="text-xs text-black">{stage.description}</p>
                </motion.div>
              )}
            </motion.div>
          )
