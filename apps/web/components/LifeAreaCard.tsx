  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-phoenix-gold/60 italic">{phoenixName}</p>
          </div>
        </div>
      ...
      <div className="flex justify-between text-sm mb-3">
        <span className="text-gray-400">Score: <span className={statusColors[status]}>{score > 0 ? '+' : ''}{score}</span></span>
        {upsets > 0 && (
          <span className="text-yellow-400/70">Upsets: {upsets}</span>
        )}
        {brokenCommitments > 0 && (
          <span className="text-red-400/70">Broken: {brokenCommitments}</span>
        )}
      </div>
